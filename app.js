...
function nav(path){
    ...
    if(arr.length > 0){
		...
		html += `<div class="mdui-toolbar-spacer"></div><div style="z-index: 289;" class="mdui-textfield"><i style="top:0.5px;right:200px"class="mdui-icon material-icons">search</i><input style="color:white; cursor:text;" id="searchinput" class="mdui-textfield-input" onkeyup="globalsearch()" type="text" placeholder="搜尋"/></div>`;
    }
    $('#nav').html(html);
}
...
function list_files(path,files){
    ...
    for(i in files){
        ...
        if(item['mimeType'] == 'application/vnd.google-apps.folder'){
            ...
            var ext = p.split('.').pop();
            if("|html|php|css|go|java|js|json|txt|sh|md|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|pdf|ppt|doc|docx|pptx|sql|".indexOf(`|${ext.toLowerCase()}|`) >= 0){
	            p += "?a=view";
	            c += " view";
            }
...
function file(path){
	...
	if("|html|php|css|go|java|js|json|txt|sh|md|sql|".indexOf(`|${ext}|`) >= 0){
	...
	if ("|pdf|ppt|doc|docx|pptx|".indexOf(`|${ext}|`) >= 0){ 
...
<div class="mdui-container-fluid">
	<br>
	<video class="mdui-video-fluid mdui-center" preload="auto" controls>
...
function file_pdf(path) {
  ...
  const inline_url = `${url}`
  const file_name = decodeURI(path.slice(path.lastIndexOf('/') + 1, path.length))
  var content = `
  <div class="mdui-container-fluid">
	<div style="max-width:950px;">
	 <embed class="mdui-center" src="https://docs.google.com/viewer?url=${inline_url}&embedded=true" style = "max-width: 100%;height:500px;width: 100%;">
	 </div>
	 <br>
    	<!-- 固定标签 -->
	<div class="mdui-textfield">
